...
#api_recommend_code=$src_code/pecoo-api-recommend
monitor_code=$src_code/cloud-monitor-server
compiled_code=$monitor_code/target/cloud-monitor-server

pro_code=/data/tomcat10000/ROOT/
#pro_code=/data/software/test
back_code=/data/software/back_code/cloud-monitor
...
compile_code $api_recommend_code pecoo-api-recommend


if [ -d $compiled_code ];then
    cd $pro_code
    tar zcvf /data/software/back_code/api_recommend/pecoo-api_recommend_`date +%Y-%m-%d-%T`.tar.gz ./*
    if [ $? -eq 0 ];then
        echo "代码备份完成。。"
    else
        echo "代码备份失败,请检查文件备份。。"
    ...
    ls -t /data/software/back_code/api_recommend/*.tar.gz |awk 'NR > 15' |xargs rm -rf
    echo "服务启动中。。"
    sleep 60
...
/home/sh/pecoo-api-recommend_recovery.sh
